...
template<> Relocaliser<float>::Relocaliser(ORUtils::Vector2<int> imgSize, ORUtils::Vector2<float> range, float harvestingThreshold, int numFerns, int numDecisionsPerFern)
{
...
template<>Relocaliser<float>::~Relocaliser(void)
{
	delete encoding;
...
template<>
bool
Relocaliser<float>::ProcessFrame(const ORUtils::Image<float> *img, const ORUtils::SE3Pose *pose, int sceneId, int k, int nearestNeighbours[], float *distances, bool harvestKeyframes) const {
...
template<> const FernRelocLib::PoseDatabase::PoseInScene & Relocaliser<float>::RetrievePose(int id)
{
	return poseDatabase->retrievePose(id);
}

template<> void Relocaliser<float>::SaveToDirectory(const std::string& outputDirectory)
{
...
template<> void Relocaliser<float>::LoadFromDirectory(const std::string& inputDirectory)
{
