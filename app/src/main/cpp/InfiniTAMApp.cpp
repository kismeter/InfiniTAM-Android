...
{
    const char *calibFile = "/sdcard/InfiniTAM/Teddy/calib.txt";
    const char *imagesource_part1 = "/sdcard/InfiniTAM/Teddy/Frames/%04i.ppm";
    const char *imagesource_part2 = "/sdcard/InfiniTAM/Teddy/Frames/%04i.pgm";
/*	const char *calibFile = "/storage/sdcard0/InfiniTAM/CAsmall/calib.txt";
    ...
    if (useLiveCamera == 0) {
        InputSource::ImageMaskPathGenerator pathGenerator(imagesource_part1, imagesource_part2);
        mImageSource = new InputSource::ImageFileReader<InputSource::ImageMaskPathGenerator>(calibFile, pathGenerator);
...
//        mImageSource = new InputSource::OpenNIEngine(calibFile);
    }
//    mMainEngine = new ITMBasicSurfelEngine<ITMSurfelT>(mInternalSettings, mImageSource->getCalib(), mImageSource->getRGBImageSize(), mImageSource->getDepthImageSize());
    mMainEngine = new ITMBasicEngine<ITMVoxel,ITMVoxelIndex>(mInternalSettings, mImageSource->getCalib(), mImageSource->getRGBImageSize(), mImageSource->getDepthImageSize());
    bool allocateGPU = false;
    if (mInternalSettings->deviceType == ORUtils::DEVICE_CUDA) allocateGPU = true;

    for (int w = 0; w < NUM_WIN; w++) {
        outImage[w] = new ORUChar4Image(mImageSource->getDepthImageSize(), true, allocateGPU);
    }

    inputRGBImage = new ORUChar4Image(mImageSource->getRGBImageSize(), true, allocateGPU);
    inputRawDepthImage = new ORShortImage(mImageSource->getDepthImageSize(), true, allocateGPU);
